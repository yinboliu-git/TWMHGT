...
# def get_Hdata(x1, x2, e, xe1=None,xe2=None, ew=None):
#     x1=x1.to(device)
#     x2=x2.to(device)
#     e=e.to(device)
#     meta_dict = {
...
def get_Hdata(x1, x2, e,xe1=None, xe2=None,em=None,ew=None):
    x1=x1.to(device)
    x2=x2.to(device)
    e=e.to(device)
    # xe1=xe1.to(device)
    ...
    meta_dict = {
        ...
        ('n1', 'e1', 'n2'): {'edge_index': e, 'edge_weight': ew},
        ('n2', 'e1', 'n1'): {'edge_index': torch.flip(e, (0, )), 'edge_weight': ew},
        # ('n1', 'n1e', 'n1'): {'edge_index': xe1, 'edge_weight': xe1w},
        # ('n2', 'n2e', 'n2'): {'edge_index': xe2, 'edge_weight': xe2w},
    }
    ...
    data[('n1', 'e1', 'n2')].edge_index = e
    data[('n2', 'e1', 'n1')].edge_index = torch.flip(e, (0, ))
    # data[('n1', 'n1e', 'n1')].edge_index = xe1
    # data[('n2', 'n2e', 'n2')].edge_index = xe2
    # data[('n2', 'n2e', 'n2')].edge_weight = xe1w
    # data[('n2', 'n2e', 'n2')].edge_weight = xe2w
...
def get_data(file_pair, params):
    ...
    if len(file_pair.mm) + len(file_pair.dd) > 0:
        for i in range(0, len(file_pair.mm)):
            ...
            xe1.append(get_self_edge(xe_1[-1], params))

        for j in range(0, len(file_pair.dd)):
            xe_2.append(pd.read_csv(file_pair.dd[i], header=None, index_col=None).values)
            xe2.append(get_self_edge(xe_2[-1], params))

        xe_1 = torch.tensor(np.array(xe_1).mean(0), dtype=torch.float32)
        xe_2 = torch.tensor(np.array(xe_2).mean(0), dtype=torch.float32)

    xe1= get_self_edge(xe_1, params)
    xe2= get_self_edge(xe_2, params)

    adj_matrix = torch.tensor(adj_matrix).to(device)
    data = get_Hdata(x1, x2, edge_index_pos, xe1, xe2,adj_matrix)

    return data, y, edg_index_all

def get_self_edge(m_matrix, params):
    m_index_pos = torch.tensor(np.column_stack(np.argwhere(m_matrix >= params.node_threshold)), dtype=torch.long).t()
    m_index_pos = torch.cat((m_index_pos, torch.flip(m_index_pos, (0,))), dim=1)
    m_edge_w = torch.tensor(m_matrix[m_index_pos[0].detach().numpy(), m_index_pos[1].detach().numpy()], dtype=torch.float32)
    m_matrix = torch.tensor(m_matrix, device=device,dtype=torch.float32)
    return m_matrix, m_index_pos, m_edge_w
